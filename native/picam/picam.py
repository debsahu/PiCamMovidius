    ...
    with picamera.PiCamera() as camera:
        with picamera.array.PiRGBArray( camera ) as frame:
            while( True ):
                ...
                infer_image( graph, img, frame.array, fps)
                end = time.time()
                seconds = end - start
                fps =  1 / seconds
...
if __name__ == '__main__':
    ...
    parser.add_argument( '-g', '--graph', type=str,
                         default=(os.path.dirname(os.path.realpath(__file__))+'/SSD_MobileNet/graph'),
                         help="Absolute path to the neural network graph file." )

    parser.add_argument( '-l', '--labels', type=str,
                         default=(os.path.dirname(os.path.realpath(__file__))+'/SSD_MobileNet/labels.txt'),
